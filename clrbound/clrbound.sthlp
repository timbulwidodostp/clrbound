...
{cmd:help clrbound}{right: ({browse "http://www.stata-journal.com/article.html?article=st0369":SJ15-1: st0369})}
{hline}

{title:Title}

{p2colset 5 17 19 2}{...}
{p2col :{cmd:clrbound} {hline 2}}Compute a one-sided bound estimate{p_end}
{p2colreset}{...}
...
{p 8 17 2}
{cmdab:clr:bound (}{it:depvar1 indepvars1 range1}{cmd:)}
...
{pstd}
{cmd:clrbound} estimates the one-sided lower- or upper-intersection bounds of
a parameter.  The variables are defined similarly as for {cmd:clr2bound}; see
{helpb clr2bound}.  {cmd:clrbound} requires the package {cmd:moremata}.
...
bound.  If {cmd:lower} is specified, {cmd:clrbound} will return the
lower-intersection bound.

{phang}
{cmd:method(}{cmd:series}|{cmd:local}{cmd:)} specifies the method of
estimation.  By default, {cmd:clrbound} will conduct parametric estimation.
If {cmd:method(series)} is specified, {cmd:clrbound} will conduct series
estimation with cubic B-splines.  If {cmd:method(local)} is specified,
{cmd:clrbound} will conduct local linear estimation.
...
linear estimation.  By default, {cmd:clrbound} calculates a bandwidth for each
inequality.  With undersmoothing, we use the "rule-of-thumb" bandwidth.  When
the {opt bandwidth(#)} is specified, {cmd:clrbound} uses the given bandwidth
as the global bandwidth for every inequality.  This option is available for
only local linear estimation.
...
particular value to another randomly, {cmd:clrbound} always initially conducts
...
{pstd}
{cmd:clrbound} stores the following in {cmd:e()}:
...
{synopt:{cmd:e(cmd)} }{cmd:clrbound}{p_end}
